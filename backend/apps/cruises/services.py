...
def effective_capacity(cruise: Cruise, cruise_date: date, override=None) -> int:
    if override is None:
...
def effective_prices(cruise: Cruise, cruise_date: date, override=None) -> tuple[Decimal, Decimal]:
    if override is None:
        override = get_date_override(cruise, cruise_date)
...
def is_bookable(cruise: Cruise, cruise_date: date, override=None) -> bool:
    if override is None:
        override = get_date_override(cruise, cruise_date)
